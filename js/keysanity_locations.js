(function(window) {
    ...
    function update_keysanity_dungeons(dungeons) {
        dungeons = update(dungeons, {
            ...
            turtle: { $merge: {
                ...
                doors: {
                    pipe: { caption: 'Pipe', can_reach: always }
                },
                locations: function() {
                    var has_big_key_access = function() { return this.keys > 1 && this.big_key },
                        laser_bridge = can_enter_and_then(function(items) {
                    ...
                    return {
                        big_key: {
                            ...
                            can_reach: function(items) { return 'unavailable'; }
                        },
                        compass: {
                            caption: 'Compass Chest',
                            second_map: true,
    ...
    // The location logic should only produce "possible" when the medallion
    // requirement isn't meet. It ranks highest since that state takes priority.
    function can_progress(items, model) {
        ...
        return max_by(states, function(state) { return ['unavailable', 'dark', 'available', 'possible'].indexOf(state); });
    }
